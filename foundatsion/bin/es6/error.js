        ...
        for (const e of msg) {
            if (typeof e === "string") {
                ...
                for (let idx_of_newline; (idx_of_newline = string_arg.indexOf("\n")) > 0;) {
                    ...
                    linebuffer = unsound.cast(`${linebuffer} ${line}`);
                    flush: new_passages.push(linebuffer);
                    linebuffer = null;
                    linebuffer;
                    continue;
                ...
                if (string_arg.length > 0) {
                    ...
                    if (linebuffer === null) {
                        ...
                        linebuffer = unsound.cast(` ${string_arg}`);
                    }
                }
...
(function (FoundatsionError) {
    ...
    (function (line) {
        ...
        function assert(u) {
            string.assert(u);
            if (u.includes("\n")) {
                throw new FoundatsionError(`Could not assert for ${this.name} because the the string`, `${text.show(u)} includes a newline!`);
            }
        }
        line.assert = assert;
        /** turns \n into "\\n" */
        ...
        line.wrap = unsound.shut_up(text.wrap);
...
})(FoundatsionError || (FoundatsionError = {}));
